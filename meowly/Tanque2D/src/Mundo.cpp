...
void Mundo::mueve()
{
	...
	//Interaccion tanque-paredes
	for (int i = 0; i < contenedor.getNumero(); i++) {
		Interaccion::colision(tankJ, *contenedor[i]);
		Interaccion::colision(tankE, *contenedor[i]);
	}
	//Interaccion con los obstáculos
	for (int i = 0; i < cajas.getNumero(); i++) {
		Interaccion::colision(tankJ, *cajas[i]);
		Interaccion::colision(tankE, *cajas[i]);
	}
